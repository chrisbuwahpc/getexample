...
!.............................................................................
    ...
    module leoF07_deferred_shp_glbls
       ...
       integer, allocatable, dimension(:), target   :: odds, evens
    ...
    ! Pointers will only reference elements with non-zero values
    ptr_odds => odds(1:MAX_EXTENT:2)
    ptr_evens => evens(2:MAX_EXTENT:2)
    ...
    CASE(2)
       ...
       !DIR$ OMP OFFLOAD target(mic) in(ptr_odds, ptr_evens)          &
       ...
       !DIR$ OMP OFFLOAD target(mic) inout(sum_OnE)                   &
                         in(offload_section_id) inout(offload_sections)
    ...
    CASE(3)
       ...
       !DIR$ OFFLOAD BEGIN target(mic)  in(ptr_odds, ptr_evens)       &
                           in(offload_section_id) inout(offload_sections)
       do i = 0,( (MAX_EXTENT / 2) -1)
          odds_n_evens(1 + (i*2)) = ptr_odds(i+1)
          sum_odds = sum_odds + ptr_odds(i+1)
       ...
       !DIR$ OFFLOAD BEGIN target(mic)  inout(sum_OnE)                  &
                           in(offload_section_id) inout(offload_sections)
       do i = 1,MAX_EXTENT
          sum_OnE = sum_OnE + odds_n_evens(i)
       enddo
    ...
    do offload_case = 1, NUM_CASES
       ...
       ! Even indexed elements = 0; Odd indexed elements = odd #
       allocate(odds(MAX_EXTENT))
       odds = 0
       odds(1::2) = (/ (i,i=1,MAX_EXTENT,2) /)
       sum_odds = 0

       ! Odd indexed elements = 0; Even indexed elements = even #
       allocate(evens(MAX_EXTENT))
       evens = 0
       evens(2::2) = (/ (i,i=2,MAX_EXTENT,2) /)
       sum_evens = 0
