...
#
         ...
         CPPFLAGS := -P -traditional
          LDFLAGS :=
#          LDFLAGS := -L/group/pawsey0001/pryan/roms -lmap-sampler-pmpi -lmap-sampler -Wl,--eh-frame-hdr
               AR := ar
          ARFLAGS := r
            MKDIR := mkdir -p
               RM := rm -f
...
ifdef USE_ARPACK
 ifdef USE_MPI
   PARPACK_LIBDIR ?= /opt/intelsoft/PARPACK
             LIBS += -L$(PARPACK_LIBDIR) -lparpack-mpif90
 endif
    ARPACK_LIBDIR ?= /opt/intelsoft/PARPACK
             LIBS += -L$(ARPACK_LIBDIR) -larpack-mpif90
endif
...
# not working for Adjoint and Data Assimlation (!) Ivica
#           FFLAGS += -ip -O3
# have to reduce optimization to O2
#           FFLAGS += -fpic -ip  -g -traceback -mcmodel=medium -assume byterecl
#           FFLAGS += -fpic -ip -O2 -g -traceback -mcmodel=medium -assume byterecl
#           FFLAGS += -fpic -ip -O2 -traceback -mcmodel=medium -assume byterecl -fp-model precise -align array32byte
           FFLAGS += -fpic -O2 -traceback -mcmodel=medium -assume byterecl -fp-model precise -align array32byte
#           FFLAGS += -fpic -g -check all -O0 -traceback -mcmodel=medium -assume byterecl -fp-model precise -align array32byte
endif
