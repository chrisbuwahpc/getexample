...
module leoF_utils
   ...
   use mic_lib         ! Provides Intel(R) MIC Architecture specific APIs
...
!* offload_check .............................................................
   ...
   !  the Target (Intel(R) MIC Architecture) and HOST executable code for 
   !  this subprogram.
...
!DIR$ NOOPTIMIZE 
     ...
     character(LEN=25), allocatable, dimension(:)    :: arg_str
...
2012 if (arg_cnt > 0 .AND. ierr /= 0) then
     ...
     print "(4X,A,/)","Checking for Intel(R) MIC Architecture (Target CPU) devices..."
     ...
     if (num_devices > 0) then
       ...
       !  Verify offload to each target works
        ...
        !  the offload code thus requires IN at least.
        ...
        do dev = 1,num_devices
           ...
           !DIR$ OFFLOAD target(mic : dev) in(offload_section_id)  &
