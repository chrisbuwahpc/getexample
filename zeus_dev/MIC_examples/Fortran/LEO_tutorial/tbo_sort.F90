...
!#******************************************************************************
!# Content:
!#      Intel(R) Fortran Composer XE 2013 Offload Demonstration
...
module tbo_glbls
   ...
   ! target_id designates the target device. -1 ensures/enforces 
...
program TBO
 ...
 ! allocate all_Vals
  ...
  !DIR$ OMP OFFLOAD target(mic : target_id) out(all_Vals)
  ...
  !DIR$ OFFLOAD target(mic : target_id) in(all_Vals) &
