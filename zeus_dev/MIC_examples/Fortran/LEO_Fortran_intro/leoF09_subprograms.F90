...
!
! 3  The two methods available to meet the requirements in items # 1 and 2
!    above are both demonstrated:
!
...
!.............................................................................
    ...
    do offload_case = 1, NUM_CASES
       ...
       CASE (1)
          ...
          !DIR$ OMP OFFLOAD target(mic) in(ival) inout(res)  &
          ...
          !DIR$ OMP OFFLOAD target(mic) in(jval)   &
                            in(offload_section_id) inout(offload_sections)
          !$omp parallel num_threads(4)
              !$omp single
       ...
       CASE (2)
          ...
          !DIR$ NOINLINE
          !DIR$ OFFLOAD target(mic) in(ival) inout(res) &
                        in(offload_section_id) inout(offload_sections)
          call leoF09_subprograms_sub(ival)
          ...
          !DIR$ NOINLINE
          !DIR$ OFFLOAD target(mic) in(jval)  &
                        in(offload_section_id) inout(offload_sections)
          res = leoF09_subprograms_fun(jval)
       ...
       CASE (3)
          ...
          !DIR$ NOINLINE
          !DIR$ OFFLOAD BEGIN target(mic) in(ival) inout(res) &
                              in(offload_section_id) inout(offload_sections)
          call leoF09_subprograms_sub(ival)
          !DIR$ END OFFLOAD 
          ...
          !DIR$ NOINLINE
          !DIR$ OFFLOAD BEGIN target(mic) in(jval)  &
                              in(offload_section_id) inout(offload_sections)
          res = leoF09_subprograms_fun(jval)
          !DIR$ END OFFLOAD 
